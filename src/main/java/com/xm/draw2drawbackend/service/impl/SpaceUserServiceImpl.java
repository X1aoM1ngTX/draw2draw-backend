...
import com.xm.draw2drawbackend.mapper.SpaceUserMapper;
...
public class SpaceUserServiceImpl extends ServiceImpl<SpaceUserMapper, SpaceUser>
     ...
     * 添加空间成员
     * 
     * @param spaceUserAddRequest 空间用户关联添加请求
     */
    @Override
    public long addSpaceUser(SpaceUserAddRequest spaceUserAddRequest) {
        // 参数校验
        ThrowUtils.throwIf(spaceUserAddRequest == null, ErrorCode.PARAMS_ERROR, "空间用户关联添加请求为空");
        SpaceUser spaceUser = new SpaceUser();
        BeanUtils.copyProperties(spaceUserAddRequest, spaceUser);
        validSpaceUser(spaceUser, true);
        // 数据库操作
        boolean result = this.save(spaceUser);
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "空间用户关联添加失败");
    ...
    public void validSpaceUser(SpaceUser spaceUser, boolean add) {
        ThrowUtils.throwIf(spaceUser == null, ErrorCode.PARAMS_ERROR, "空间用户关联为空");
        // 创建时，空间ID和用户ID必填
     ...
     * 获取空间成员包装类（列表）
     * 
     * @param spaceUserList 输入列表
     * @return List<SpaceUserVO> 列表
     */
    @Override
